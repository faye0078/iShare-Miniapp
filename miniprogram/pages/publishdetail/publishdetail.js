...
Page({
  ...
  onLoad(options) {
    ...
    this.setData({
      ...
      postMessage:getApp().globalData.postMessage,
      openid: getApp().globalData.openid
    })
      for(var i=0;i<this.data.postMessage.length;i++){
      if(this.data.postMessage[i]._id == options.messageId){
        this.setData({
        targetMessage: this.data.postMessage[i],
        })
      }
    }
    if(getApp().globalData.openid==this.data.targetMessage._openid){
      this.setData({
        isOwner: true
      })
    }
